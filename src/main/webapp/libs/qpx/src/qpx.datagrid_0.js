    ...
    var DataGrid = qpx.Widget.extend({
        ...
        _doAdaptiveLayout: function () {
            ...
            if (!needAdaptive) {
                ...
                // v popupu není nic skryté -> ikona pro jeho otevření se nezobrazuje
                this.$tbody.find(".qpx-datagrid-cell-adaptive").hide();
                this._closeAdaptiveAccordion();
                return;
            }
            ...
            // ikona pro otevření popupu se zobrazí jen tehdy, pokud se do něj
            // skutečně nějaký sloupec přesunul (jinak by v popupu nebylo co zobrazit)
            var anyAdaptiveHidden = cols.some(function (c) {
                return c.visible !== false && c.adaptiveHidden;
            });
            this.$tbody.find(".qpx-datagrid-cell-adaptive").toggle(anyAdaptiveHidden);
            if (!anyAdaptiveHidden) {
                this._closeAdaptiveAccordion();
            }
        },
